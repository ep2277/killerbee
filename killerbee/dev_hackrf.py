...
class HackRF:
    def __init__(self, dev: str) -> None:
        '''
        Instantiates the KillerBee class for the HackRF platform running GoodFET firmware.
        @type dev:   String
        @param dev:  Serial device identifier (ex /dev/ttyUSB0)
        @return: None
        @rtype: None
        '''
        'self.packet_queue is used to store a packet when pnext() is called and a packet is already available.'
    ...
    # KillerBee expects the driver to implement this function
    def inject(self, packet: bytes, channel: Optional[int] = None, count: int = 1, delay: int = 0,
               page: int = 0) -> None:
        '''
    ...
    def pnext(self, timeout: int = 100) -> Any:
        ...
        if rssi is None:
            ...
            result[
                'dbm'] = rssi - 45  # TODO tune specifically to the Apimote platform (does ext antenna need to different?)
        return result
